...
order by page_view_number rows between unbounded preceding and unbounded following " %} with sessionized as
  (with pageviews as
     (select *
      from {{pages_target}} {% if is_incremental() %}
 ...
 --multiple devices.
 select *,
        row_number() over (partition by anonymous_id
                           order by timestamp) as page_view_number
      from pageviews),
        lagged as
 ...
 select *,
        lag(timestamp) over (partition by anonymous_id
                             order by page_view_number) as previous_tstamp
      from numbered),
        new_sessions as
 ...
 select *, {{ dbt_utils.datediff('previous_tstamp', 'timestamp', 'second') }} as period_of_inactivity, 
                                                                            (case 
                                                                                 when {{ dbt_utils.datediff('previous_tstamp', 'timestamp', 'second') }} <= {{inactivity_cutoff}} then 0 
                                                                                 else 1 
                                                                             end) as new_session 
      from lagged), 
        session_numbers as 
 ...
 --calculations.
 select *,
        sum(new_session) over (partition by anonymous_id
                               order by page_view_number rows between unbounded preceding and current row) as session_number
      from new_sessions),
        session_ids as
     (--This CTE assigns a globally unique session id based on the combination of
 --anonymous_id and session_number.
 select *, {{dbt_utils.surrogate_key('anonymous_id', 'session_number')}} as session_id
      from session_numbers) select *
   from session_ids),
                                                                                             pageviews as
  (select distinct session_id,
                   anonymous_id, {%
   for (key,
        value) in first_values.items() %} first_value({{key}}) over ({{window_clause}}) as {{value}}, {% endfor %} {%
   for (key,
        value) in last_values.items() %} last_value({{key}}) over ({{window_clause}}) as {{value}}, {% endfor %} min(timestamp) over ({{partition_by}}) as session_start_tstamp,
                                                                                                                                     max(timestamp) over ({{partition_by}}) as session_end_tstamp,
                                                                                                                                                         count(*) over ({{partition_by}}) as page_views
   from sessionized),
                                                                                             sessionized_durations as
  (select *, {{ dbt_utils.datediff('session_start_tstamp', 'session_end_tstamp', 'second') }} as duration_in_s
   from pageviews {% if is_incremental() %}
  ...
  (select *,
   ...
   from sessionized_durations {% if is_incremental() %}
   where session_start_tstamp >
       (select {{ dbt_utils.safe_cast(dbt_utils.dateadd('hour', -2, 'max(session_start_tstamp)'), 'timestamp') }}
        from {{ this }}) {% endif %} ),
                                                                                             id_stitching as
  (select distinct anonymous_id,
                   last_value(user_id) over (partition by anonymous_id
                                             order by case
                                                          when user_id is not null then 0
                                                          else 1
                                                      end ASC, timestamp rows between unbounded preceding and unbounded following) as user_id,
  ...
  (select sessions.*,
   ...
   from sessions
   left join id_stitching using (anonymous_id) {% if is_incremental() %}
   where session_start_tstamp > {{sessionization_cutoff}} {% endif %} ) {% if is_incremental() %} , agg as
  (select blended_user_id,
          count(*) as starting_session_number
   from {{this}} -- only include sessions that are not going to be resessionized in this run

   where session_start_tstamp <= {{sessionization_cutoff}}
   group by 1) {% endif %}
select *,
       row_number() over (partition by blended_user_id
                          order by s.session_start_tstamp) {% if is_incremental() %}+ agg.starting_session_number {% endif %} as session_number
from stitched_sessions s {% if is_incremental() %}
left join agg using (blended_user_id) {% endif %}