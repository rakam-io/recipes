...
(
    ...
    {{
        dbt_utils.safe_cast(
            dbt_utils.dateadd(
                'hour',
                -%(segment_sessionization_trailing_window)0.2f,
      ...
      new_sessions as (
        ...
        select
          *,
          ...
          case
          when
          -- segment_inactivity_cutoff
          {{ dbt_utils.datediff('previous_tstamp', 'timestamp', 'second') }} <= (%(segment_inactivity_cutoff)0.2f then 0 else 1 end as new_session
        from lagged
